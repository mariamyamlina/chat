...
class MessageService: MessageServiceProtocol {
    ...
    func save(messages: [Message],
        ...
        coreDataStack.performSave { [weak self] context in
            ...
            for message in messages {
                guard coreDataStack.load(message: message.identifier, from: context, errorHandler: errorHandler) == nil else { continue }
    ...
    func save(message: Message,
        ...
        coreDataStack.performSave { context in
            guard let channel = coreDataStack.load(channel: channel.identifier, from: context, errorHandler: errorHandler) else { return }
            guard coreDataStack.load(message: message.identifier, from: context, errorHandler: errorHandler) == nil else { return }
            let messageDB = MessageDB(identifier: message.identifier,
                                      content: message.content,
                                      created: message.created,
                                      senderId: message.senderId,
    ...
    private func delete(compareWithMessages messages: [Message],
        ...
        coreDataStack.performSave { context in
            ...
            coreDataStack.arrayDifference(entityType: .message, predicate: channelDB.identifier, arrayOfEntities: messages, in: context, errorHandler: errorHandler).forEach {
