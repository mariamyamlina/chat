...
class ChatHelper {
    ...
    static var messages = {(person: String) -> [(MessageTableViewCell.MessageCellModel, UIColor, Date)] in
        ...
        case "Leonardo DaVinci":
            ...
            messages.append((MessageTableViewCell.MessageCellModel(text: "A few months ago we wrote to you that we would like to have a young Christ, at the age of 12, written in your hand; you have replied through Messer Angelo Tovaglia that you will gladly fulfill our request; but due to too many orders that you have to fulfill, we doubt if you remember ours. So, we decided to write you these few lines"), greenColor, Date(timeIntervalSinceNow: -7000)))
        ...
        case "Jennifer Aniston":
            ...
            messages.append((MessageTableViewCell.MessageCellModel(text: "I was invited to an event, but I can't choose what to wear. There are clear regulations, and I need to pick up clothes in two days. This is really a problem for me"), greenColor, Date(timeIntervalSinceNow: -150)))
        ...
        case "Vladimir Putin":
            messages.append((MessageTableViewCell.MessageCellModel(text: "We see how the situation with the coronavirus epidemic in the world is developing sharply. In many countries, the number of cases continues to grow. The entire world economy was under attack, and its decline is already predicted. Thanks to measures taken in advance, in general, it is still possible to contain both wide and rapid spread of the disease. Russia - simply even in its geographical position - cannot isolate itself from threats. There are states near our borders that are already seriously affected by the epidemic, and it is objectively impossible to block its penetration into our country - so it is to work professionally, in an organized and proactive manner. And the main priority here is the life and health of our citizens."), greyColor, Date(timeIntervalSinceNow: -4000)))
        ...
        case "Adolf Hitler":
            messages.append((MessageTableViewCell.MessageCellModel(text: "During the formation of the second troops away from the eyes and aviation, as well as in connection with the recent operations in the Balkans, a large number of my troops, about 80 divisions, accumulated along the border with the Soviet Union, which, possibly, gave rise to circulating rumors about a possible military conflict between us. I assure you as a head of state this is not the case. For my part, I also understand that you cannot completely ignore these rumors. It's about one month. Around June 15-20, I plan to begin a massive transfer of troops to the West from your border. At the same time, in the most convincing way, you do not succumb to any provocations that may occur on the part of my generals who have forgotten their duty. And it goes without saying, try to give them no reason. If provocation from one of my generals cannot be avoided, please, show restraint, do not take retaliatory actions and immediately report what happened via the communication channel known to you"), greyColor, Date(timeIntervalSinceNow: -5300000)))
